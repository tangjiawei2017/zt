	...
	$(function() {
		layui.use([ 'table', 'layer' ], function() {
			...
			table.render({
				...
				skin : 'line',
				limit : 10,
				url : "${ctx}/customForm/listAjax.action",
				method : "post",
			});
			//监听工具条
			table.on('tool(custom_from_filter)', function(obj) {
				...
				if (layEvent === 'detail') { //查看
					window.open("${ctx}/customForm/CustomFormDetail.action?id="+data.id,"_blank");
				} else if (layEvent === 'del') { //删除
					layer.confirm('真的删除该该表单么?', {
						'title' : '提示'
					}, function(index) {
						layer.close(index);
						//向服务端发送删除指令
						$.ajax({
							url : "${ctx}/customForm/delCustomForm.action?id="
									+ data["id"],
							type : "POST",
							dataType:"json",
					...
					layer.open({
						type : 2,
						title : '编辑表单',
						content :"${ctx}/customForm/preEditCustomForm.action?id="+data.id,
						...
						yes : function(index, layero) {
							...
							if(result.status){
								layer.close(index);   //关闭加载页面
							 	$.ajax({
									url:"${ctx}/customForm/updateCustomForm.action",
		...
		$(".add-customFrom-btn").click(function() {
			...
			layer.open({
				type : 2,
				title : '新增表单',
				content :"${ctx}/customForm/preAddCustomForm.action",
				offset: [height+'px', width+'px'],
				area : [ '950px', '540px' ],
				btn : [ '保存', '取消' ],
				btnAlign : 'c',
				yes : function(index, layero) {
					...
					if(result.status){
						layer.close(index);   //关闭加载页面
					 	$.ajax({
							url:"${ctx}/customForm/addCustomForm.action",
							data:{"data":JSON.stringify(result.data)},
							dataType:"json",
							type:"POST",
							success:function(data){
