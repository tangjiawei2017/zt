...
<script>
    ...
    //计算商品库存,校验货品库存
    function checkAndcalculateProductStock(obj){
    	var stock = $.trim($(obj).val());
    	var reg = /^\d+$/;
    	if(!reg.test(stock)){
    		layer.msg("货品库存输入不合法!");
    		obj.focus();
    ...
    function calculateProductStock(){
    	var totalStock=0;
    	$(".product-item-spec input[name='itemStock']").each(function(){
			 totalStock += Number($.trim($(this).val()));
		});
		$("input[name='stock']").val(totalStock);
    }
    
    //设置货品的上下架
    function updateSaleStatus(obj){
    	if($(obj).data("sale")==1){
    ...
    //通过value值计算规格索引 spec0,spec1,spec2
    function calculateSpecIndexByValue(value){
    	var result={
    		defaultText:"",
    		index:0
    	};
       	$("input[name^=spec]").each(function(){
       		if($(this).data("value")==value){
       			result.index=$(this).parent().data("index");
       			result.defaultText=$(this).val();
       		}
       	});
       	return result;
    }
	...
	$(function() {
		...
		priceFormat();
		//删除
		$(".img-div").children('.product-img-delete').on('click', function(){
		  removeByValue(images,$(this).parent().children("img").data("src"));
		  $(this).parent().remove();
		});
		...
		layui.use('upload',function() {
            ...
            editor.create();
            //初始化数据
			editor.txt.html($("textarea[name='description']").val());
		    //表单初始化校验
		    $("#form").formValidation();
		    //加载商品分类
			$.ajax({
			...
			$(".add-relate-btn").click(function(){
				parent.layer.open({
					type : 2,
					title:'选择商品',
					content : "${ctx}/pages/product_relate.jsp?id=${product.id}",
					area : [ '800px', '540px' ],
					btn : [ '保存', '取消' ],
	...
	/**  
	  * 主要用于 商品编辑时,18会显示为 18.00情况
	  * 去掉多余的"0"和"."
	  * @param str
	*/
	function priceFormat() {
		$price=$("input[name='price']");
		$marketPrice=$("input[name='marketprice']");
		$price.val(parseFloat($price.val()));
		$marketPrice.val(parseFloat($marketPrice.val()));
	}
	//扩展属性初始化赋值
	function initProductExtendAttr(){
		var extArray=[];
		extArray.push("${product.ext0}");
		extArray.push("${product.ext1}");
		extArray.push("${product.ext2}");
		extArray.push("${product.ext3}");
		extArray.push("${product.ext4}");
		extArray.push("${product.ext5}");
		extArray.push("${product.ext6}");
		extArray.push("${product.ext7}");
		extArray.push("${product.ext8}");
		extArray.push("${product.ext9}");
		for(var i=0;i<extArray.length;i++){
			if(typeof extArray[i]!='undefined' && extArray[i]!=null && extArray[i]!=''){
				if($('.extendattr input[name="ext'+i+'"]').length>0){
					var $this=$('.extendattr input[name="ext'+i+'"]');
					if($this.length==1){
						if($this.is(":checkbox")){
						...
						var dataArray=$.trim(extArray[i]).split(",");
						for(var j=0;j<dataArray.length;j++){
							$this.each(function(){
								if($(this).val()==dataArray[j]){
									$(this).attr("checked","checked");
								}
							});
					    }
					}
				}else if($('.extendattr select[name="ext'+i+'"]').length>0){
					$('.extendattr select[name="ext'+i+'"]').val(extArray[i]);
				}
			}
		}
	}
	...
	window.checkData = function() {
		...
		//排序码处理,数值不正确的,以100处理
		var sortCode = $.trim($("input[name='sortCode']").val());
		if (sortCode == '' || Math.floor(sortCode) != sortCode
				|| Math.floor(sortCode) < 0) {
			$("input[name='sortCode']").val(100);
		}
		//商品扩展属性 必填项判断
				...
				<form class="layui-form" action="" id="form">
					<fieldset style="min-height: 250px; border: 1px solid #d9e0e8;">
						...
						<div class="layui-form-item name-module">
							...
							<div class="layui-input-inline">
								<input type="text" name="name" class="layui-input" value="${product.name}"
						...
						<div class="layui-form-item code-module">
							<label class="layui-form-label"><span class="showName">商品编号</span>:<span
								class="need">*</span></label>
							<div class="layui-input-inline">
								<input type="text" name="code" class="layui-input" value="${product.code}"
									data-rule='{"ajax":"${ctx}/product/checkProductCodeExist.action?id=${product.id}","errorMsg":"该商品编号已存在!"}'>
							</div>
							<div class="layui-form-mid shop-check-msg"></div>
						</div>
						<div class="layui-form-item category-module">
							...
							<div class="layui-input-inline">
								<input type="hidden" name="categoryid" value=""> <input
									type="text" name="category_id" value="${product.productCategory.name}"
									data-value="${product.productCategory.id}" class="layui-input shop-select"
							...
							<div class="layui-input-inline">
								<select name="brandid">
									<c:forEach items="${brandList}" var="brand">
										<option value="${brand.id}" <c:if test="${product.brandid==brand.id}">selected</c:if>>${brand.name}</option>
									</c:forEach>
								</select>
							</div>
						</div>
						<div class="layui-form-item">
							<label class="layui-form-label"><span class="showName">市场价</span>:<span
								class="need">*</span></label>
							<div class="layui-input-inline">
								<input type="text" name="marketprice" class="layui-input" value="${product.marketprice}"
						...
						<div class="layui-form-item">
							<label class="layui-form-label"><span class="showName">库存</span>:<span
								class="need">*</span></label>
							<div class="layui-input-inline">
								<input type="text" name="stock" class="layui-input" value="${product.stock}"
					...
					<fieldset style="min-height: 250px; border: 1px solid #d9e0e8;">
						...
						<div class="layui-upload">
							...
							<div class="layui-upload-list">
								<div class="product-img-div" id="demoList">
								    <c:if test="${product.images!=''}">
									    <c:set value="${ fn:split(product.images, ',') }" var="productImages" />
									    <c:forEach items="${productImages}" var="productImg">
											     <script> images.push("${productImg}");</script>
											     <div class="img-div" ><img src="${productImg}" data-src="${productImg}"/><i class="product-img-delete"></i></div>
									    </c:forEach>
								    </c:if>
								</div>
							</div>
					...
					<fieldset class="product-relate-div"
						...
						<legend>相关商品:</legend>
							...
							<table class="layui-table product-relate-table">
								...
								<tbody>
								  <c:forEach items="${productRelateList}" var="relate">
								      <tr><td><input type="hidden" name="id" value="${relate.id}">
								         <span>${relate.name}</span></td><td>${relate.code}</td><td>
								         <a href="javaScript:void(0)" style="color:blue" onclick="removeRealteProduct(this)">删除</a></td></tr>
								  </c:forEach>
								</tbody>
							</table>
					...
					<div id="editor"></div>
					<textarea name="description" style="display: none;">${product.description}</textarea>
				</form>
			</div>
		</div>
	</div>
