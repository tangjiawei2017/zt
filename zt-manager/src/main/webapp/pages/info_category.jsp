...
var zTreeSetting = {
	...
	$(function() {
		...
		$(".add-btn").click(function() {
			layer.open({
				type : 2,
				title:'新增资讯分类',
				content : "${ctx}/info/preAddCategory.action",
				area : [ '680px', '500px' ],
				scrollbar :false,
				btn : [ '保存', '取消' ],
				yes : function(index, layero) {
					...
					if(result.status){
						...
						$.ajax({
							url:"${ctx}/info/addCategory.action",
							data:result.data,
							type:"POST",
							dataType:"json",
							success:function(data){
								refresh("${ctx}/info/showCategory.action",1,data.result);
							}
						});
					}
				},
	...
	function editCategory(id){
		layer.open({
			type : 2,
			title : '编辑资讯分类',
			content : "${ctx}/info/preEditCategory.action?id="+id,
			area : [ '680px', '500px' ],
			btn : [ '保存', '取消' ],
			yes : function(index, layero) {
				var childFrame = window.frames["layui-layer-iframe" + index];    //得到加载层页面window对象
				...
				if(result.status){
					layer.close(index);   //关闭加载页面
					$.ajax({
						url:"${ctx}/info/updateCategory.action",
						data:result.data,
						type:"POST",
						dataType:"json",
						success:function(data){
						    refresh("${ctx}/info/showCategory.action",2,data.result);
						}
					});
				}
			}
	...
	function delCategory(id){
		$.ajax({
			url:"${ctx}/info/delCategory.action?id="+id,
			type:"post",
			dataType:"json",
			success:function(data){
				if(data.result=="success"){
					...
					//重新加载数据
					$.ajax({
						url : "${ctx}/info/showCategory.action",
						type : "POST",
						async : "true",
						cache : false,
						success : function(data) {
	...
	function addChildCateogry(parentid){
		layer.open({
			type : 2,
			title:'新增资讯分类',
			content : "${ctx}/info/preAddCategory.action?parentid="+parentid,
			area : [ '800px', '540px' ],
			btn : [ '保存', '取消' ],
			yes : function(index, layero) {
				var childFrame = window.frames["layui-layer-iframe" + index];    //得到加载层页面window对象
