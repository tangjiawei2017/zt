...
var setting = {
	...
	$(function() {
		...
		$("#form").formValidation();
		$.ajax({
			url:"${ctx}/product/productCateogryListAjax.action?id=${productCategory.id}",
            type:"post",
            dataType:"json",
            success:function(zNodes){
                ...
                var id=$.trim($("input[name='parentCategory'").data("value"));
				...
				<c:when test="${productCategory!=null}">
					 <form class="layui-form" action="" id="form">
						...
						<div class="layui-form-item">
							...
							<div class="layui-input-inline">
								<input type="text" name="name" 
									class="layui-input" value="${productCategory.name}"
									data-rule='{"ajax":"${ctx}/product/checkProductCategoryExist.action?id=${productCategory.id}","errorMsg":"该分类名称已存在!"}'>
							</div>
						...
						<div class="layui-form-item">
							...
							<div class="layui-input-inline">
								<c:if test="${productCategory.parent==null}">
								...
								<c:if test="${productCategory.parent!=null}">
									<input type="text" name="parentCategory" value="${productCategory.parent.name}"
										data-value="${productCategory.parent.id} " class="layui-input shop-select" readonly
										onclick="showMenu(); return false;">
									<i class="ces-input-select" id="showcateControl"
										onclick="showMenu(); return false;"></i>
								</c:if>
						...
						<div class="layui-form-item img-div">
							...
							<div class="layui-input-inline image">
					           <button type="button" class="layui-btn layui-btn-normal" id="logo">选择</button>
					           <c:if test="${productCategory.image!=null}">
								<section class="up-section logo-img">
									<span class="up-span"></span> <img class="close-upimg"
										src="${ctx}/images/del.png"> <img class="up-img" src="${productCategory.image}" data-src="${productCategory.image}" />
								</section>
								</c:if>
							</div>
						</div>
						...
						<div class="layui-form-item">
							<label class="layui-form-label">绑定模板:*</label>
							<div class="layui-input-inline">
								<select name="templateid">
								    <c:forEach items="${templateList}" var="template">
								       <option value="${template.id}" <c:if test="${template.id == productCategory.templateid}">selected</c:if>>${template.name}</option>
								    </c:forEach>
								</select>
							</div>
						</div>
						<div class="layui-form-item layui-form-text">
								...
								<div class="layui-input-block">
									<textarea name="content" class="layui-textarea">${productCategory.content}</textarea>
								</div>
							</div>
					</form>
				</c:when>
					...
					<form class="layui-form" action="" id="form">
						<div class="layui-form-item">
							...
							<div class="layui-input-inline">
								<input type="text" name="name" lay-verify="required"
									class="layui-input"
									data-rule='{"ajax":"${ctx}/product/checkProductCategoryExist.action","errorMsg":"该分类名称已存在!"}'>
							</div>
							<div class="layui-form-mid shop-check-msg"></div>
						</div>
						<div class="layui-form-item">
							...
							<div class="layui-input-inline">
							   <c:if test="${parentid==null}">
								<input type="text"  name="parentCategory"
									value="顶级展示分类" data-value="0" class="layui-input shop-select" readonly  onclick="showMenu(); return false;"> <i
									 class="ces-input-select"
									id="showcateControl" onclick="showMenu(); return false;"></i>
								</c:if>
								<c:if test="${parentid!=null}">
									<input type="text" name="parentCategory" value="${parentName}"
										data-value="${parentid}" class="layui-input shop-select" readonly
										onclick="showMenu(); return false;">
									<i class="ces-input-select" id="showcateControl"
										onclick="showMenu(); return false;"></i>
								</c:if>
						...
						<div class="layui-form-item">
							...
							<div class="layui-input-inline">
								<select name="templateid">
								    <c:forEach items="${templateList}" var="template">
								       <option value="${template.id}" <c:if test="${template.isdefault}">selected</c:if>>${template.name}</option>
								    </c:forEach>
								</select>
							</div>
						</div>
